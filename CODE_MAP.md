...
- `Domain/UniquePay.cs`: UniquePay Bearer/form-urlencoded client, owned-wallet settlement, fail-closed authoritative verification for both official toman fee-payer contracts, durable settlement claims, restricted provisional OWNED credits, and the polling reconciliation hosted service. New invoice creation is single-attempt; inquiry is bounded and read-only.
