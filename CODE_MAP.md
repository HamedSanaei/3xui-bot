...
- Destination handling (`Domain/Logging/TelegramDestination.cs`) — one shared parser/validator decides what a Telegram log destination is: a numeric chat id (including negative `-100…` supergroup/channel ids) or an `@username`. Blank and the numeric zero sentinel are reported as *missing*; any other text is *malformed*. A bare word is never repaired into a username, because inventing a destination would misroute operational logs. `AppConfig.BackupChannel` is a `long` whose missing default is 0, so `BotRegistry.NormalizeBot` (and `BotRegistry.ToConfig` for rows already persisted by an older build) now normalizes blank/`0` to an empty string; the string `"0"` must never become a destination field again. `Program.cs` sanitizes the legacy `loggerChannel`/`backupChannel` keys and both backup-channel candidates before precedence is applied, and `WeeklyUsageReportHostedService` resolves its logger channel through the same validator instead of constructing `ChatId` from raw configuration. A durable Payment/Html event with a missing or malformed logger channel is **not** queued: `TelegramLogger` emits one throttled local-only diagnostic (a private static throttle, at most one per reason per minute, capped to four tracked reasons, console/journal only, never routed through Telegram) instead of creating an undeliverable Pending row — while a Payment event still records its durable `DatabaseBackupState` generation via `TelegramLogDispatcher.RequestBackupIntent`, which reuses the exact payment-row SQL through the shared `IncrementBackupRequestAsync` helper. Backup generation semantics are therefore **unchanged**: one payment still requests exactly one generation, whether or not its audit line could be admitted. Rows already persisted are re-validated at dispatch time: an unusable destination throws the dedicated `TelegramDestinationInvalidException`, which `TelegramRateLimitPolicy.IsPermanentFailure` treats as permanent and `TelegramLogDispatcher` **dead-letters immediately** with the stable `LastError = invalid_logger_destination` (row retained for inspection, never retried forever); `IsTransientFailure` explicitly returns `false` for it, and a plain `ArgumentException` is deliberately NOT classified as a permanent Telegram failure because it may be a programming bug. Startup calls `ConfigurationPreflight.DescribeStartupReport`, which prints sanitized warnings when no deliverable logger or backup channel resolves and never throws — a logging misconfiguration must never stop payment settlement, XUI work, or customer handling.
...
- All production data access uses factories with short-lived contexts (`UserDbContextFactory`,
  ...
  registered anywhere.
- A Telegram log destination is configuration, not data. Never treat a non-blank string as "configured": validate it
  through `TelegramDestination` (or `Sanitize`/`SelectValid`) before it reaches `ChatId` or an outbox row. In
  particular `AppConfig.BackupChannel` is a `long` whose default is `0` and must never be rendered as the string
  `"0"`, and `configuration["backupChannel"] = ""` throws in the configuration binder because the key is numeric —
  absence is the only supported "unset" form for that legacy key. A durable row whose destination can never be
  delivered must be dead-lettered with `invalid_logger_destination`, never retried as a transient failure, because a
  local `ArgumentException` from Telegram.Bot has no attachable Telegram response and would otherwise keep the row
  Pending forever while the real configuration fault stays hidden.
- `Data/configuration.json` is production state and a real outage was caused by a human pruning it against an example
  taken from another branch, which deleted `xuiV3ApiBaseUrl` and the per-bot `loggerChannel`/`backupChannel` values.
  Never prune or rebuild it from a key list (`jq '{known keys}' … > new-file`), never treat
  `Data/configuration.example.json` as a schema — it intentionally omits deployment-specific sections and its own
  `_readme` block says so — and make configuration edits additive and targeted. `jq` also rejects `//` comments, which
  is why the example's warning lives in a `_readme` key rather than in JSON comments. Startup validation is
  `ConfigurationPreflight`; it never writes config and never rejects an unknown key.
