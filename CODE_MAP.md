...
- A bot becomes deliverable through exactly two pieces of state, and neither is `TelegramSenderService.ActiveBots`. Membership is `BotRegistry` (id, type, `Enabled`): a bot is deliverable only if the registry exposes that exact id as enabled, which is what the pump filters on. Pipeline registration is the sender's lane set, added on the first job handed off for a bot and logged once as `Telegram sender lane registered`. `ActiveBots` is only the number of bots with a job in flight at that instant, so a quiet storefront legitimately shows `ActiveBots=1` while the owned bot carries the operator-log stream; it must never be read as a registration count. Owned, assistant, and tenant bots all use the same path (`MultiBotHostedService.StartBotCoreAsync` -> `BotClientProvider.GetClient(botId)` -> `ForegroundBoundedTelegramBotClient` -> `TelegramSenderService.EnqueueAsync(botId, ...)`). Diagnostics: `Telegram output census` (registry/lane/queue shape, Information only when it changes) and a bounded per-bot warning `Telegram output is queued for a bot this pipeline cannot deliver to. Reason=…` for output that can never be selected, with the rows deliberately retained. `TelegramSenderService.DescribeLaneIneligibility` is the single rule for that reason (`bot_disabled`, `bot_id_mismatch`, `bot_not_found`-style absence is reported as `bot_id_mismatch` because `BotRegistry.GetById` answers an unknown id with the default owned bot).
- `BotRegistry` must never take an `ILogger` dependency. Production registers an `ILoggerProvider` whose factory resolves `BotRegistry`, so injecting `ILogger<BotRegistry>` closes the cycle registry -> logger factory -> provider -> registry and hangs the container (observed as a full-suite stall/crash). Membership diagnostics therefore live at the callers: application startup logs the hydration census and the tenant ids that entered the registry (`LastLoadAddedIds`/`LastLoadReplacedIds`), `MultiBotHostedService.StartAsync` logs the receiver-vs-registry census, and the sender logs lanes and undeliverable output. `BotRegistry.Describe()`/`DescribeIds()` are the read-only snapshots those callers use.
- Exactly one long-polling loop may exist per internal `BotId` and per Telegram token. `Services/TelegramPollingLeaseRegistry.cs` is the process-local single-flight guard: `TryAcquire` checks and inserts under one lock and refuses a duplicate immediately (never queues) with a closed-vocabulary `TelegramPollingLeaseRejection` plus the holding bot id, token fingerprint prefix, generation, and process id. A lease lives exactly as long as its loop — it is released by the loop's own completion (`OnReceiverLoopCompleted`), or, for an intentional restart, by the restarter after `TelegramReceiverLifetime.ObservePreviousAsync` has proven the predecessor loop ended (`Release` is generation-checked, so a late predecessor can never free a replacement's lease). A restart may only wait for a lease whose holder was marked `Stopping` by `StopBotCore`/`StopAsync`; an unmarked holder is a hard duplicate rejection. `LogConfiguredPollingIdentityAudit` additionally reports, before any receiver starts, how many configured runtime bots share one token — that configuration mistake produced the production symptom of several unrelated storefronts failing at once, because Telegram answers the second poller with its 409 conflict. A start that throws *after* registration now stops through `StopBotCore` so teardown has one owner instead of cancelling/disposing a token the live loop was still using.
- `Domain/Logging/TelegramPollingFailureDiagnostics.cs` is the single polling-failure classifier (closed-vocabulary `TelegramPollingFailureKind`): `GetUpdatesConflict`, `WebhookConflict`, `RateLimited`, `Transient`, `Delivery`, `Unknown`. Every polling log line now carries `botId`, `tokenHashPrefix`, `pollingGeneration`, `pollingTaskId`, `processId`, `failureKind`, `errorType`, `innerException`, `statusCode`, `responseText`, and a cancellation reason. Conflict classification is evaluated *before* the rate-limit/delivery/transient branches so a conflict can never be answered with a retry delay, and it recognizes Telegram's documented conflict text and a bare HTTP 409 as well as a typed `ApiRequestException` error code — the previous classifiers required `ApiRequestException`, which is why a plain `RequestException` 409 (edge response body not parseable as Telegram JSON) surfaced as the untriageable `Telegram polling degraded errorType=RequestException` line. `TelegramRateLimitPolicy.IsRateLimited` likewise accepts a plain `RequestException` whose HTTP status is 429, so that shape keeps the authoritative `RetryAfter` pause instead of falling through. Response text is single-lined, length-capped, and token-redacted before logging; the token pattern uses explicit lookarounds (not `\b`) because the common leak shape is a token concatenated onto a letter in a URL path (`/bot<token>/getUpdates`).
