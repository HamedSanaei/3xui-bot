...
فایل نمونه فقط یک قالب توضیحی است و فهرست کامل کلیدها نیست. هرگز فایل `Data/configuration.json` تولید را بر اساس آن هرس (prune) نکنید و با `jq` از فهرست کلیدها بازسازی نکنید؛ کلیدهای ضروری مثل `xuiV3ApiBaseUrl` و `loggerChannel` حذف می‌شوند و بات دیگر نمی‌تواند به پنل وصل شود یا لاگ بفرستد.
